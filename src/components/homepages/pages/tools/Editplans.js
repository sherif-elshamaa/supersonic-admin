...
import { posttoast, editplans } from "../../../../JS/Actions/actions"
import baseUrl from '../../../../utils/baseUrl'

function Editplans({ open, setOpen, item }) {
    ...
    const handleChangePlan = async (e) => {
        ...
        if (price === "") {
            ...
            setErrorMsg("Please enter a price before sumbit")
            return
        }
        try {
            const { data } = await axios.put(
                `${baseUrl}/api/admin/editplan`,
                { id: item._id, price: price },
                { withCredentials: true }
            )
            dispatch(editplans(data.plan))
            dispatch(posttoast({ toast: { state: 'success', text: 'Plan price updated successfully!', show: true } }))
            setFormLoading(false)
            setOpen(false)
            setPrice("")
        } catch (error) {
                                    ...
                                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                                        ...
                                        <div className="mt-2">
                                            <p className="text-sm text-gray-500 mb-4">
                                                Are you sure you want to change {item.type} plan price?
                                            </p>
                                            <div className="flex">
                                                <label className="my-auto mr-2">New Price: </label>
                                                <input
                                                    type="number"
                                                    className="mt-1 bg-gray-100 focus:ring-gray-500 focus:border-gray-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                                    value={price}
