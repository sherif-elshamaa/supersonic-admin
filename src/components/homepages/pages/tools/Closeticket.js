...
import { posttoast, closeticket } from "../../../../JS/Actions/actions"
import baseUrl from '../../../../utils/baseUrl'

function Closeticket({ open, setOpen, item }) {
    ...
    const handleCloseticket = async (e) => {
        ...
        try {
            const { data } = await axios.put(
                `${baseUrl}/api/admin/closeticket`,
                { id: item._id, text: text },
                { withCredentials: true }
            )
            dispatch(closeticket(data.ticket))
            dispatch(posttoast({ toast: { state: 'success', text: 'Ticket closed successfully!', show: true } }))
            setFormLoading(false)
            setOpen(false)
            setText("")
        } catch (error) {
                                        ...
                                        <div className="mt-2">
                                            ...
                                            <div className="flex">
                                                ...
                                                <textarea
                                                    ...
                                                    value={text}
                                                    rows="4" cols="50"
                                                    onChange={handlePriceChange} ></textarea>
                                            </div>
                                        </div>
                                    </div>
                                </div>
